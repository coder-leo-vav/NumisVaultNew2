/**
 * @file Banknotes.jsx
 * @description Страница галереи банкнот с поиском, фильтрами и управлением
...
/**
 * Страница галереи банкнот
 * @returns {JSX.Element} Страница с коллекцией банкнот
 */
export default function BanknotesPage() {
  ...
  const { data: collectibles = [], isLoading } = useQuery({
    queryKey: ['collectibles', 'banknote'],
    queryFn: () => base44.entities.Collectible.filter({ type: 'banknote' }, '-created_date'),
  });
  ...
  const handleSave = (data) => {
    if (editingItem) {
      ...
      createMutation.mutate({ ...data, type: 'banknote' });
    }
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
          ...
          <p className="text-[#86868B] mt-1">
            {collectibles.length} {collectibles.length === 1 ? 'банкнота' : 'банкнот'} в коллекции
          </p>
        </div>
        <Button 
      ...
      {isLoading ? (
        ...
        <EmptyState type="banknote" onAdd={() => { setEditingItem(null); setFormOpen(true); }} />
      ) : (
        <div className={cn(
          "grid gap-4",
          viewMode === 'grid' 

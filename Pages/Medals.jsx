/**
 * @file Medals.jsx
 * @description Страница галереи медалей с поиском, фильтрами и управлением
...
/**
 * Страница галереи медалей
 * @returns {JSX.Element} Страница с коллекцией медалей
 */
export default function MedalsPage() {
  ...
  const { data: collectibles = [], isLoading } = useQuery({
    queryKey: ['collectibles', 'medal'],
    queryFn: () => base44.entities.Collectible.filter({ type: 'medal' }, '-created_date'),
  });
  ...
  const handleSave = (data) => {
    if (editingItem) {
      ...
      createMutation.mutate({ ...data, type: 'medal' });
    }
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
          ...
          <p className="text-[#86868B] mt-1">
            {collectibles.length} {collectibles.length === 1 ? 'медаль' : 'медалей'} в коллекции
          </p>
        </div>
        <Button 
      ...
      {isLoading ? (
        ...
        <EmptyState type="medal" onAdd={() => { setEditingItem(null); setFormOpen(true); }} />
      ) : (
        <div className={cn(
          "grid gap-4",
          viewMode === 'grid' 
